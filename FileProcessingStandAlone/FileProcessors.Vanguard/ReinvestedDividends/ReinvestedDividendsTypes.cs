...
public class ReinvestedDividends : PhysicalRecord
{
...
public string FileIdentifier { get; set; } // max length 10; required False
public string FileNumber { get; set; } // max length 2; required False
public string SecurityNumber { get; set; } // max length 9; required False
...
public DateTime PayableDateFormatted
{
	get
	{
		...
		{
			_payableDateFormatted = DateTime.Parse(Formatter.Format(PayableDate, @"(\d{2})/(\d{2})/(\d{2})~20$3/$1/$2~DateTime"));
		}
		return (DateTime)_payableDateFormatted;
	}
}

public string AggregatedReinvestedDividendAmount { get; set; } // max length 18; required False
private Decimal? _aggregatedReinvestedDividendAmountFormatted;
public Decimal AggregatedReinvestedDividendAmountFormatted
{
	get
	{
		if (_aggregatedReinvestedDividendAmountFormatted == null)
		{
			_aggregatedReinvestedDividendAmountFormatted = Decimal.Parse(Formatter.Format(AggregatedReinvestedDividendAmount, @"([-\d]\d{15})(\d{2})~$1.$2~Decimal"));
		}
		return (Decimal)_aggregatedReinvestedDividendAmountFormatted;
	}
}
...
public class PayableDateSummary : BlockingRecord
{
public string RecordId { get; set; } // max length 2; required False
public string FileIdentifier { get; set; } // max length 10; required False
public string FileNumber { get; set; } // max length 2; required False
...
[System.Xml.Serialization.XmlInclude(typeof(PayableDateSummary))]
