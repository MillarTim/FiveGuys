	...
	public void EndOfFileEventProcessor(ref FileCompletionInfo fileCompletionInfo)
		{
			...
			if (ErrorRecords > 0)
			{
				string error = string.Format("Error processing file {0}, errors encountered on {1} out of {2} records. See errors in log for more details.",
					FileName,
					ErrorRecords,
					TotalRecords);
		...
		private void GetInfoFromHeaders(string headerXml)
		{
			...
			UtcTradeCapture utcTradeCapture =
				(UtcTradeCapture)(new XmlSerializer(typeof(UtcTradeCapture))).Deserialize(new StringReader(headerXml));

			FileTimestamp = utcTradeCapture.DateTimeCreatedFormatted;
			int fileNumber = 0;
			int.TryParse(utcTradeCapture.ApplicationMultiCycleCounter, out fileNumber);
