	...
	public class FileEventProcessors : IFileEventProcessors
	{
		...
		public void EndOfFileEventProcessor(ref FileCompletionInfo fileCompletionInfo)
		{
			...
			try
			{
				...
				string transactionType = fileService.GetTypeMapping("022-ALL", "TRT");
				string accountNumber = fileService.GetTypeMapping("022-ALL", "ACC");
				if (transactionType == null || accountNumber == null)
				{
					didErrorOccur = true;
					error = "File Identifier 022-ALL not found in TypeMappings.";
				}
				string trailer = string.Format("022, {0} File Input", FileNumber);
